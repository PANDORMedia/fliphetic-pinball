// bridge/server.js
// Serial-to-HTTP bridge for the ESP32 button box. Reads the ESP's JSON-line
// output from the serial device and:
//   GET /events  — Server-Sent Events: each ESP line is pushed instantly
//                  (low latency; the game uses this for flipper input).
//   GET /state   — the most recent line as a one-shot snapshot.
// nginx exposes these to the game as /esp/events and /esp.
//
// Dependency-free: pure Node, plus busybox `stty` to set the line mode. If
// the ESP is absent the bridge still runs and serves {}.
